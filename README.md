...
- 使用稳定客户端会话建立一次 target 绑定：未绑定建立时对完整 target 做 HRW 排序并按冻结顺序 fallback；匿名与认证会话+模型均固定到 tier/凭证/池/模型/协议/上游地址（代理为带代际 fencing 的当前选择，可在同池同凭证同模型同协议同地址内移动，同移动保持 route/wire 会话与 prompt_cache_key 不变）；首次成功后不再跨 Tier/跨凭证/跨池 fallback，也不再从匿名升级到认证；429 在同绑定内走当前可发送代理（当前优先、稳定亲和顺序、本地限流冷却跳过、不受 retry.max_attempts 截断），其他失败按目标协议本地返回，避免把已建立上下文换 target 重放
- 代理传输故障只影响传输健康；401 按 tier+凭证全局冷却，429 按（通道，代理池，代理节点）全局冷却（匿名与认证共享 Zen 通道；同 URL 不同池隔离），同一 tier+凭证仅在其本请求冻结 eligible 全集实际 429 穷尽后才额外冷却该凭证（取最后 Retry-After，单个 eligible 穷尽亦写；部分 429 与预冷却跳过不计证据、其他凭证/通道候选不受影响）；403/5xx 只冷却单个 (tier, credential, pool, proxy, model) target，且仅在同凭证另有节点成功作比较时才写入通道级 403/5xx 冷却，否则仅展示；408/425/400 与普通 4xx 中性
...
2. 模型 ID 大小写不敏感地包含 `free`；即使 metadata 尚未就绪、缺少该模型或显示为付费，也仍按名称条件视为免费。
 ...
 免费模型先走匿名 Zen，非免费模型完全跳过匿名通道。未绑定建立阶段候选分散与 fallback 由冻结顺序负责（HRW/round-robin 决定 target 顺序，fallback 按顺序走下一个）；同 target retry 只负责临时性验证，从不负责分散。匿名通道每个可用 target 最多一次 fallback 发送，整个通道另有唯一共享 transient token：首次传输错误、408/425 或 5xx 在同一 target、同 Route Session、同 body 上追加一次 transient retry；401/403/429 无同 target retry，直接按冻结顺序 fallback，其中 429 按代理池 + 代理节点全局冷却并过滤所有模型/凭证的后续候选，而 429 链不受 `retry.max_attempts` 截断（认证 429 同样 budget-neutral，走尽当前可发送 credential×proxy 候选）；普通 4xx（404/422 等，排除 400/401/403/429/408/425）直接结束匿名通道（不再扫剩余代理），随后可进入认证 Key 阶段；匿名耗尽（无 400）后仍进入认证，全部原生路由最终 429 耗尽才可进入自定义最终保底（见自定义 session fallback）。任意 candidate 首次精确 HTTP 400 走专用恢复：同一 candidate 保持同 Route Session 精确重放一次（同 request ID、attempt +1、同 target/协议/credential/proxy、同 wire 会话字节，始终在向客户端写任何字节之前；Responses 重放同时清理 `previous_response_id` 与 `input[]` 中的 `reasoning` 旧引用；不消耗 transient token，不存 override），重放结果即整条路由最终结果，不再扫剩余代理、不进入认证 Tier；该重放是同 target 陈旧引用恢复，不是换 target 的许可，重放成功若仍未绑定则建立会话绑定。认证 lane 使用独立 transient token；精确 400 同样同会话单次重放且终结整条路由，普通 4xx 结束当前 lane（无跨 Tier 回退）。已绑定会话固定到 tier/凭证/池/模型/协议/上游地址（匿名与认证一致，代理为代际 fencing 当前选择）：429 在同绑定内走尽当前可发送代理（本地限流冷却跳过、无新证据），成功则只更新当前代理；全耗尽按目标协议保留最后 429（含 Retry-After）或进入自定义保底；其他失败按目标协议本地返回。客户端取消或请求总 deadline 立即终止整条路由，不再 retry/fallback，不改变任何状态。监控中的 `proxy_node` 表示所选代理节点、`proxy_pool` 表示其所属池，两者都不代表或推断实际出口 IP。
...
| 字段 | 含义 |
| --- | --- |
| `retry.max_attempts` | 每个认证 Key Tier 的普通（非 429）真实发送预算，含首次发送与同 target transient retry（仅未绑定建立阶段按冻结顺序 fallback 有效；429 发送 budget-neutral，走尽当前可发送 credential×proxy 候选、不受本预算截断）。同一 Tier 内首次传输错误、408/425 或 5xx 共享一次同 target transient retry（同 Route Session、同 body）；401/403/429 无同 target retry，直接按冻结顺序 fallback，其中 429 在记录全局代理限流后 fallback（同池同代理的后续候选已被过滤，未绑定新会话直接过滤该代理全部候选；同一凭证仅冻结 eligible 全集实际 429 穷尽后写 credential429，取最后 Retry-After）；普通 4xx 结束当前 Tier。任意 candidate 首次精确 400 走同会话重放一次，该次固定额外允许、不受本预算截断，重放结果即路由最终结果、不再走剩余候选或回退另一 Tier，重放成功若仍未绑定则建立绑定。已绑定会话不跨凭证/池/通道：只保留同 target transient retry 与精确 400 同 target 同会话重放，429 在同绑定内走尽可发送代理（全耗尽保留最后 429 或进入自定义保底），其他冷却按目标协议本地失败。单认证 lane 内无跨 Tier 回退；精确 400 重放终结整条路由。anonymous 429 同样不受此上限，每个可用 target 最多一次 fallback 发送，另加全通道一次 transient retry 与可选一次同会话 400 重放；普通 4xx 直接结束匿名通道。 |
...
- 没有显式会话标识时，使用第一条用户消息生成稳定客户端会话 ID，使同一段多轮对话保持一致；客户端会话是建立身份，从不原文出进程：未绑定时用于稳定排序并按冻结顺序 fallback，400 恢复不修改它、不重排已冻结候选；首次成功后同一会话+模型建立绑定，匿名与认证均绑定 tier/凭证/池/模型/协议/上游地址并允许同池内 429 移动（代理为代际 fencing 当前选择，同移动保持 route/wire 会话与 prompt_cache_key），之后不再跨 Tier/跨凭证/跨池回退。
- 如果两个独立会话的第一条消息完全相同，建议由客户端发送不同的 `x-session-id`，以确保两个会话严格分离。
- 上游实际发送的是按 route target scope 分离的 Route Session（上游 authority、tier、credential 内部身份、proxy pool、target protocol，不含 model、不存原始 credential；两通道 scope 均与代理无关，同池移动保持同一上游会话值与 body 字节）：内部仍为 target-bound 非原文的 `rss_*`（首代稳定无状态派生，store 仅保留 legacy/migrated override、无 live 路径新建 override；精确 400 重放保持同会话、不 rotate/不存 override），线上编码为与之稳定对应的规范 OpenCode 形伪名 wire ID（`ses_` 规范形状、`msg_` 请求、`40hex` 项目、父会话亦为规范伪名），写入 `x-opencode-session` / `x-opencode-request` / `x-opencode-project` / 规范父会话，并同步覆盖已存在的 body `conversation_id`、`metadata.session_id` 为同一 wire 会话（缺失不新增，类型不符明确报错）；Responses 还会将 `prompt_cache_key` 置为同一 wire 会话、`store` 缺失时默认 `false`（显式值保留）。Apply 会按原生 proxy-free target scope 有效性/新鲜度过滤迁移 route override（旧 proxy-bound 丢弃重派），重启丢失；override 不进日志/metrics/history/admin。会话+模型 target 绑定与此不同：它是 durable 建立绑定，Apply 无有效性过滤迁移为 tombstone 身份（被删除/变化的 target 仍保持绑定并本地 502，不重建），重启清空；429 在同绑定内走尽当前可发送代理（本地限流冷却跳过、全耗尽保留最后 429 或进入自定义保底），其他失败按目标协议本地返回。
- 可用性探测（原生与自定义）使用同样的官方集合与确定性规范会话/请求/项目值，无状态且不写 scheduler；自定义 fallback 推理与探测同样发送规范伪名化 OpenCode 路由元数据以兼容要求 session 的供应商，但每次仍发送完整历史、不依赖供应商保存会话内容；自定义可用性保留精确 HTTP 状态观察（`http_status`）。
