...
自定义 session fallback（OpenAI-compatible chat）：配置 `fallback: {"active":"name","channels":[{"name":"...","base_url":"https://...","api_key":"...","model":"..."}]}`，命名唯一，active 可空、非空必须引用现有渠道，URL 仅 http/https。仅当请求进入时已存在匿名 session+model pin 且在 pinned 匿名路径得 HTTP 429（含本地 proxy429 冷却 429）时触发；未绑定匿名 429 与认证 pin 429 不触发。无 active 时保持原 429；有 active 时当次经当时 active 渠道重试（客户端入口统一转 chat 并改写为渠道模型，经 `{base}/v1/chat/completions` Bearer 发送，不依赖供应商会话亲和，每次带完整历史；响应/流转回客户端协议；自定义模型不进公开 `/v1/models`；不碰 Zen/Go 调度冷却），且无论成功与否先将会话固化到该完整渠道身份（name+规范化 base URL/authority+key 指纹/身份+配置模型），此后该会话仅走该固化渠道、不随 active 切换漂移；自定义错误原样返回不回退。接管键为 session，有界 4096 first-wins，容量满本地 502；热 Apply 无过滤迁移为 tombstone，删除/身份变化后旧会话本地 502；进程期、无持久化/投影/过期，重启清空。WebUI 支持增删/改渠道、经供应商 GET `/v1/models` 拉模型下拉并选用、一键激活，激活经配置保存/应用持久化。
...
| `POST` | `/api/availability/check` | 批量可用性检测（全部渠道真实最小推理）：原生按目录实际可服务模型与原生协议 POST `stream:false` 约 1 token，自定义渠道用其配置模型测 chat；请求体必须为严格空对象 `{}`；每客户端每分钟最多 3 次，同时只允许一次运行（忙时 `409`）。 |
| `POST` | `/api/fallback/discover` | 自定义渠道模型发现（GET `{base}/v1/models` 仅下拉）：支持未保存新 base_url+明文 key 与已保存 masked secret id；限流 10/分钟，限大小/超时，验证模型 id，`no-store` 不泄露 key。 |
